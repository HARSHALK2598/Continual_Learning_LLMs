...
def orthogonal_lora_loss(model, past_task_weights, device, normalized = False):
    # print(past_task_weights)
    orth_loss = torch.tensor(0.0).to(device)
    if len(past_task_weights) > 0:
        for task_name, saved_loraA_params in past_task_weights.items():
            for saved_layer_name, saved_layer_param in saved_loraA_params.items():
                for model_layer_name, model_layer_param in model.named_parameters():
                    if saved_layer_name in model_layer_name: # the current model parameters have prefix due to accelerator wrapper.
                        
                        if not normalized:
                            term = (torch.abs(torch.mm( model_layer_param,  saved_layer_param.to(device).T))).sum()
                            term = term / model_layer_param.shape[0]
                        else:
                            normalized_mlp = model_layer_param / torch.norm(model_layer_param, dim = 1, p=2).reshape(-1,1) # r x hidden_dim
                            normalized_slp = saved_layer_param / torch.norm(saved_layer_param, dim = 1, p=2).reshape(-1,1) # r x hidden_dim
                            term = (torch.abs(torch.mm( normalized_mlp,  normalized_slp.to(device).T))).sum() # do I need square?
                            term = term / model_layer_param.shape[0]
                        # print("MATCHED AND ADDED")
                        orth_loss += term
                        break
    return orth_loss
...
if __name__ == '__main__':
    ...
    for task in args.tasks:
        if accelerator.is_main_process:
            logger.info("Preparing the %s dataset for continual training."%task)
    ...
    for i, (train_task_name, train_dataloader) in enumerate(train_dataloaders.items()):
        ...
        for epoch in range(1, args.train_epochs+1):
            ...
            train_loss, train_orth_loss = train(model, train_dataloader, optimizer, lr_scheduler, 
                                                        args.llambda, olora_past_A_matrices, accelerator, args.normalized_lora)
            if accelerator.is_main_process:
    ...
    # with open(os.path.join(args.experiment_dir,'olora_adapters.pickle'), 'wb') as handle:
    #     pickle.dump(olora_past_A_matrices, handle, protocol=pickle.HIGHEST_PROTOCOL)
