    ...
    for i, (train_task_name, train_dataloader) in enumerate(train_dataloaders.items()):
        ...
        if args.lora_target_modules:
            peft_config = LoraConfig( task_type=TaskType.CAUSAL_LM, inference_mode=False, bias="none",
                ...
                lora_dropout=args.lora_dropout, fan_in_fan_out=args.lora_fan_in_fan_out, use_dora=True  # Enabling DoRA)
        else:
            peft_config = LoraConfig( task_type=TaskType.CAUSAL_LM, inference_mode=False, bias="none",
                r=args.lora_r, lora_alpha=args.lora_alpha, lora_dropout=args.lora_dropout, 
                fan_in_fan_out=args.lora_fan_in_fan_out, use_dora=True  # Enabling DoRA)
